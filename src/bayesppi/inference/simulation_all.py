...
META_CSV = Path(r"C:\Users\AV75950\Downloads\all_people_7_20_2025.csv")
meta = pd.read_csv(META_CSV)  # sep="," is the default

# 2) Clean column names: strip spaces and replace internal spaces with underscores
meta.columns = (
    meta.columns
        .str.strip()             # remove leading/trailing spaces
        .str.replace(' ', '_')   # e.g., "Acq Date" → "Acq_Date"
)
...
# Now extract only subject_id, label, Acq_Date
...
# 6) Save
merged.to_csv("matched_cn_ad_labels3.csv", index=False)
print("✅ Saved to matched_cn_ad_labels3.csv")
