...
Coverage & interval width vs. number of labels (ALL setting).
- Headless matplotlib backend (Agg) for CI/docs
- CLI-driven; reads CSV or falls back to a baked-in default dataset
...
--------------------------------------
ppi_n, ppi_cov, ppi_w     # each row may repeat (will be uniqued), or appear as one row per n

Examples
--------
# 1) Built-in defaults (uses your latest numbers) → save PNGs to ./figures
python coverage_interval_all.py --out figures/

# 2) From CSV (no PPI overlay)
python coverage_interval_all.py --csv data/coverage_all.csv --out figures/

# 3) From CSV + PPI arrays via CLI (comma-separated)
python coverage_interval_all.py --csv data/coverage_all.csv --out figures/ \
  --ppi-n 10,20,40,80 --ppi-cov 0.96,0.94,0.90,0.90 --ppi-w 0.371070,0.245060,0.169302,0.119885
"""
...
def load_data(csv_path: Path | None) -> Tuple[pd.DataFrame, Optional[pd.DataFrame], bool]:
    ...
    Returns:
      ...
      used_default: whether we used baked-in defaults
    """
    if csv_path is not None:
        df = pd.read_csv(csv_path)
        logging.info("Loaded CSV: %s (n=%d)", csv_path, len(df))

        # Split out optional PPI columns if present
        ppi_cols = {"ppi_n", "ppi_cov", "ppi_w"}
        has_ppi = ppi_cols.issubset(set(map(str.lower, df.columns)))
        df_main = df.copy()
        ...
        if has_ppi:
            ...
            df_ppi = df[[cols["ppi_n"], cols["ppi_cov"], cols["ppi_w"]]].copy()
            df_ppi.columns = ["ppi_n", "ppi_cov", "ppi_w"]
            logging.info("Detected PPI columns in CSV; will overlay PPI line.")

        return df_main, df_ppi, False

    # Built-in default dataset (your updated numbers)
    data = {
    ...
    df_ppi = pd.DataFrame({
        "ppi_n":   [10, 20, 40, 80],
        "ppi_cov": [0.96, 0.94, 0.90, 0.90],
        "ppi_w":   [0.371070, 0.245060, 0.169302, 0.119885],
    })
...
def main() -> None:
    ...
    plot_coverage(
        df=df,
        out_path=out_dir / "coverage_vs_labels_all.png",
        dpi=args.dpi,
    ...
    plot_width(
        df=df,
        out_path=out_dir / "interval_width_vs_labels_all.png",
        dpi=args.dpi,
