...
Make a reproducible figure contrasting an exemplar AD case and an exemplar CN case
selected from autorater predictions, similar in spirit to the original snippet.

Repository/TMLR-friendly upgrades:
- No hard-coded paths (everything via CLI)
- Headless plotting (matplotlib Agg); saves to file, no plt.show()
- Robust CSV validation and flexible column names
- Derives `H` from a label column if a binary H column is not present
- Safe NIfTI loading with nibabel and slice normalization
- Deterministic artifact (PNG/SVG/PDF)

Expected columns in the input CSV
---------------------------------
- prediction column (default `autorater_prediction`) — float in [0, 1]
- nifti path column (default `nifti_path`) — path to .nii or .nii.gz
- and either:
  - numeric `H` in {0,1}, or
  - categorical `label` with tokens mapping to AD=1 and CN=0

Examples
--------
# Basic (defaults assume standard column names)
python model_adcn.py \
  --csv data/processed/autorater_predictions_all4.csv \
  --out figures/model_adcn_extremes.png

# With custom columns and tokens
python model_adcn.py \
  --csv data/processed/preds.csv \
  --pred-col p_ad --nifti-col path --label-col diagnosis \
  --ad-token AD --cn-token CN \
  --out figures/model_adcn_extremes.svg

# If a numeric H already exists
python model_adcn.py \
  --csv data/processed/preds.csv \
  --h-col H \
  --out figures/model_adcn_extremes.pdf
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Render an AD vs CN exemplar figure from autorater predictions.")
...
def load_dataframe(
    ...
    if h_col and h_col in cols:
        ...
        if df["H"].isna().any():
            ...
            logging.warning(
                "Derived H has %d missing rows (labels not matching %s/%s). Dropping them.",
                n_na, ad_token, cn_token
            )
